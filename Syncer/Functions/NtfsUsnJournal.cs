...
namespace chenz
{
    public class NtfsUsnJournal : IDisposable
    {
        ...
        /// <summary>
        /// 在卷上创建USN日志。如果USN日志已存在且需要更大的日志文件空间，
        /// 函数将调整MaximumSize 和 AllocationDelta参数。
        /// </summary>
        ...
        public UsnJournalReturnCode GetFilesMatchingFilter(string filter, out List<WINAPI.UsnEntry> files)
        {
            DateTime startTime = DateTime.Now;
        ...
        /// <summary>
        /// 根据文件索引号（reference number）得到文件的完整路径。
        /// </summary>
        ...
        /// <summary>
        /// 传入一个先前的USN日志状态，函数判断USN日志是否激活并且日志入口尚未丢失（换句话说，USN日志有效）。
        /// 随后函数读取USN日志入口集合。如果函数未返回USN_JOURNAL_SUCCESS，该列表为空。
        /// </summary>
        ...
        /// <summary>
        /// 将一个Win32错误转化为UsnJournalReturnCode。
        /// </summary>
